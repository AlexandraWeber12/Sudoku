...
public:
	...
	void prepareGrid() {
        ...
        for (int x = 0; x < dim; x += sqrt(dim)) {
            for (int y = 0; y < dim; y += sqrt(dim)) {
    ...
    bool checkRow(int const col, int val) {
    ...
    bool checkCol(int const row, int val) {
    ...
    bool checkSection(int const row, int const col, int val) {
        int sqrtDim = sqrt(dim);
	...
	void solveSudoku(int cntFields = 0) {
        for (auto element : DataArray[CoordinateArray[cntFields].x][CoordinateArray[cntFields].y].possibleValues) {
            if (checkRow(CoordinateArray[cntFields].x, element) && 
